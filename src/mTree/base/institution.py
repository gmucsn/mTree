class Institution:
    def __init__(self, environment, name=None):
        self.environment = (
            environment  # environment in which the institution is invoked
        )
        ...
        self.open = (
            True  # could possibly be used as a flag for accepting available players
        )
    ...
    def receive_event(
        self, event_id, event_data=None
    ):  # methods must have a default parameter that can be set to None
    ...
    def start_institution(self, debug=False):
        if debug:
            print("WARNING: You have not defined start_institution()")
        pass

    def close_institution(self, debug=False):
        if debug:
            print("Closing Institution")
