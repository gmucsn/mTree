...
class AuctionInstitution(Institution):
    def prepare(self):
        ...
        self.split_size = math.floor(4 / 2)
        self.bids = []
        self.bid_history = []
        self.log_message("inst 10 TESTING THIS...", target="trial", level=10)
        self.log_message("inst 66 TESTING THIS...", target="trial", level=66)

        self.log_message("inst 10 TESTING THIS...", level=10)
        self.log_message("inst 66 TESTING THIS...", level=66)

        self.log_message("inst TESTING THIS...", target="trial")
    ...
    def start_bidding(self):
        ...
        for agent in agents:
            ...
            value_estimate = random.uniform(
                self.common_value - self.error, self.common_value + self.error
            )
            new_message.set_payload(
                {"value_estimate": value_estimate, "error": self.error}
            )
