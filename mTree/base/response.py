...
class Response:
    ...
    def show_all(
        self, item_id
    ):  # why is the user_id passed here? With broadcast=TRUE it should just go to all.
        """
    ...
    def get_querystring_item(self, user_id, querystring_item):
        ...
        """
        self.emit(
            "willow_action_response",
            {"data": {"action": "get_parameter", "item": querystring_item}},
            room=user_id,
            namespace=self.namespace,
            callback=self.get_querystring_callback,
        )
    ...
    def set_user(
        self, user_id, item_id, property_name, property_value
    ...
    def open_html(self, file):
        ...
        context = {}
        template = (
            Environment(loader=FileSystemLoader(self.template_location or "./"))
            .get_template(file)
            .render(context)
        )
        return template
    ...
    def update_admin_status(self, msg):  # What dis do?
        self.emit("admin_action", msg, broadcast=True, namespace="/admin")
