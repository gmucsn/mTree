...
def launch_background_actor_system():
    ...
    process = Popen(
        [sys.executable, background_actor_py, "true"]
    )  # , stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) #, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) #, stdout=PIPE, stderr=PIPE)
