...
ADMIN_PASSOWRD = "mTreePassword" #MTree_configuration.instance.admin_password
...
def login(response: Response, admin_credentials: Annotated[str, Form()], request: Request):
    if admin_credentials == ADMIN_PASSOWRD:
        response.set_cookie(key="admin-user", value="ADMIN-USER")        
        return RedirectResponse("/developer/status")
    else:
        return templates.TemplateResponse(
            "admin_login.html", {"request": request, "error": "Invalid Credentials"}
        )

@developer_router.post("/status", tags=["developer"])
def status(request: Request):
    # try:
    title = "Status"
    return templates.TemplateResponse(
            "status.html", {"request": request, "title": title}
        )


@developer_router.get("/subject_runs", tags=["developer"])
def subject_runs(request: Request):
    ...
    print("\t\t", simulations)
    return templates.TemplateResponse(
        "mes_human_subject_configurations.html", {"request": request, "title": title,
        "simulations":simulations, "mes_directory":mes_directory}
    )



@developer_router.get("/mes_human_subject_configuration_view", tags=["developer"])
def mes_human_subject_configuration_view(request: Request, mes_directory: str, configuration: str):
    ...
    return templates.TemplateResponse(
        "mes_human_subject_configuration_view.html", {"request": request, "title": title,
        "simulation":simulation, "mes_directory":mes_directory,
        "configuration": configuration}
    )


@developer_router.get("/mes_run_human_subject_experiment", tags=["developer"])
def mes_run_human_subject_experiment(request: Request, mes_directory: str, configuration: str):
...
def human_subject_status(request: Request, configuration: str):
    title = "Human Subject Status"
    return templates.TemplateResponse(
        "human_subject_status.html", {"request": request, 
        "title": title,
        "subject_ids":SUBJECT_IDS,
        "configuration": configuration}
    )
    # return render_template(
    #     "human_subject_status.html",
    #     title=title,
    #     configuration=configuration,
    #     subject_ids=SUBJECT_IDS,
    # )
...
#     else:
#         system_run_output = component_registry.get_system_run_output_by_run_code(
#             run_code
#         )

#     return render_template(
#         "view_system_run_output.html", system_run_output=system_run_output
#     )
...
#     system_run_log_dir = request.args.get("system_run_log_dir")
#     component_registry = Registry()
#     mes_configuration = component_registry.get_system_run_configuration(
#         system_run_log_dir
#     )
#     return render_template(
#         "display_mes_run_configuration.html", simulation=mes_configuration
#     )
...
#     return render_template(
#         "display_mes_run_sequence_diagram.html",
#         system_run_output=system_run_output,
...
#     return render_template(
#         "simulation_library.html", simulation_library=simulation_library
#     )
...
#     return render_template(
#         "simulation_configuration_viewer.html", simulation=simulation
#     )
