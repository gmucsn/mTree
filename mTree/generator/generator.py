...
class Generate(object):
    ...
    def ask_project_questions(self):
        ...
        try:
            self.project_template = ProjectTemplate(project_name=project_name, agent_name=agent_name, institution_name=institution_name, environment_name=environment_name)
            return True
        except ValidationError as e:
            text = Text()
            text.append("There are errors with your project information\n", style="bold red")
    ...
    def construct_directory(self):
        ...
        text = Text()
        text.append(f"Attempting to create new project directory: {self.project_directory}\n", style="bold green")
        self.console.print(text)    
        ...
        if not self.project_directory.exists():
            ...
            text = Text()
            text.append(f"\tThe project directory has been created: {self.project_directory}\n", style="bold green")
            
            return True
        else:
            text = Text()
            text.append(f"The target directory already exists: {self.project_directory}\n", style="bold red")
            self.console.print(text)
    ...
    def copy_agent_template(self):
        ...
        with open(
            self.mes_directory / self.agent_name, "w"
        ) as fileout:
    ...
    def copy_institution_template(self):
        ...
        f = pkg_resources.resource_stream(__name__, path)
        self.institution_name = self.project_template.institution_name + "_institution.py"
        self.institution_class_name = self.project_template.institution_name.title() + "Institution"
        template = f.read().decode('utf-8')
        template = template.replace("NAME", self.institution_class_name)
        with open(
            self.mes_directory / self.institution_name, "w"
        ) as fileout:
            fileout.write(template)
    ...
    def copy_environment_template(self):
        ...
        f = pkg_resources.resource_stream(__name__, path)
        self.environment_name = self.project_template.environment_name + "_environment.py"
        self.environment_class_name = self.project_template.environment_name.title() + "Environment"
        template = f.read().decode('utf-8')
        template = template.replace("NAME", self.environment_class_name)
        with open(
            self.mes_directory / self.environment_name, "w"
        ) as fileout:
            fileout.write(template)
    ...
    def create_basic_configuration(self):
        ...
        with open(
            self.config_directory / "basic_simulation.json", "w"
        ) as fileout:
            fileout.write(template)
