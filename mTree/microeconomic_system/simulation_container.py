...
class SimulationContainer:
    ...
    def create_actor_system(self):
        logcfg = {
            ...
            "loggers": {
                "": {"handlers": ["h1", "h2", "exp", "json"], "level": logging.DEBUG}
            },
        }
        ...
        self.actor_system = ActorSystem(
            "multiprocTCPBase", capabilities
        )  # , logDefs=logcfg)
        actorSys = self.actor_system or ActorSystem(
            "multiprocTCPBase", capabilities
        )  #'multiprocQueueBase')
        try:
            actorSys.tell(actorSys.createActor(SimpleSourceAuthority), "register")
        except:
    ...
    def create_dispatcher(self):
        ...
        try:
            self.dispatcher = ActorSystem("multiprocTCPBase", capabilities).createActor(
    ...
    def send_dispatcher_simulation_configurations(self, configurations):
        ...
        for configuration in configurations:
            ...
            ActorSystem("multiprocTCPBase", capabilities).tell(
                self.dispatcher, configuration_message
            )
    ...
    def send_root_environment_message(self, environment_name, message):
        ...
        ActorSystem("multiprocTCPBase", capabilities).tell(
            self.environments[environment_name], message
        )
