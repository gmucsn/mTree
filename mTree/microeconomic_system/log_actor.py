...
class LogActor(Actor):
    ...
    def __str__(self):
        return (
            "<LogActor: " + self.__class__.__name__ + " @ " + str(self.myAddress) + ">"
        )
    ...
    def log_data(self, message):
        if message.target is not None:
            ...
            with open(os.path.join(data_target), "a") as file_object:
                ...
                file_object.write(
                    str(message.get_timestamp()) + "\t" + json_data + "\n"
                )
        else:
    ...
    def log_message(self, message):
        if message.target is not None:
            ...
            with open(os.path.join(log_target), "a") as file_object:
                file_object.write(
                    str(message.get_timestamp())
                    + "\t"
                    + str(message.get_content()).replace("\n", "  ")
                    + "\n"
                )
    ...
    def setup_log_files_folder(self):
        ...
        self.output_log_folder = os.path.join(
            log_container_directory,
            self.simulation_run_id + "-R" + str(self.run_number),
        )
        ...
        self.log_target = os.path.join(
            self.output_log_folder,
            self.simulation_run_id + "-R" + str(self.run_number) + "-experiment.log",
        )
        self.data_target = os.path.join(
            self.output_log_folder,
            self.simulation_run_id + "-R" + str(self.run_number) + "-experiment.data",
        )
        self.tmp_log_target = os.path.join(
            self.output_log_folder,
            self.simulation_run_id
            + "-R"
            + str(self.run_number)
        ...
        self.tmp_data_target = os.path.join(
            self.output_log_folder,
            self.simulation_run_id
            + "-R"
            + str(self.run_number)
        ...
        self.sequence_target_tmp = os.path.join(
            self.output_log_folder,
            self.simulation_run_id + "-R" + str(self.run_number) + "-sequence.log.tmp",
        )
        self.sequence_target = os.path.join(
            self.output_log_folder,
            self.simulation_run_id + "-R" + str(self.run_number) + "-sequence.log",
        )
        ...
        self.configuration_target = os.path.join(
            self.output_log_folder,
            self.simulation_run_id
            + "-R"
            + str(self.run_number)
    ...
    def create_mes_status_file(self):
        ...
        at a previous run
        """
        self.mtree_mes_status_file = os.path.join(
            self.output_log_folder, ".mtree_mes_status.json"
        )
    ...
    def write_mes_exception(self, exception_payload):
        self.mtree_mes_exception_file = os.path.join(
            self.output_log_folder, "mes_exception_information.json"
        )
        ...
        if not isinstance(message, ActorSystemMessage):
            try:
                ...
                log_message = LogMessage(
                    message_type="log", content=traceback.format_exc()
                )
                self.log_message(log_message)
