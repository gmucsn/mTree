...
class MonitorClient:
    def __init__(self, handled_logging_levels):
        ...
        self.match_maker_addr = self.asys.createActor(
            MatchMaker, globalName="MatchMaker"
        )
    ...
    def start_monitoring(self):
        ...
        while True:
            ...
            if isinstance(msg, LogMsg):
                heapq.heappush(
                    self.m_tuples, (msg.time.timestamp(), np.random.rand(), msg)
                )
                clear()
                for msg_tuple in heapq.nsmallest(
                    len(self.m_tuples), self.m_tuples, lambda m_tuple: m_tuple[0]
                ):
                    msg = msg_tuple[2]
