...
class Registry:
    ...
    def get_system_runs(self, run_dir=None):
        ...
        p = Path(base_dir)
        system_runs = [
            self.read_system_run_status_file(run_status_file)
            for run_status_file in p.glob("**/.mtree_mes_status.json")
        ]
        return system_runs
    ...
    def add_class(self, classobject):
        ...
        class_source = "temp"
        Registry.instance.class_list[class_name] = {
            "class": classobject,
            "source": class_source,
        }
    ...
    def get_mes_component_details(self, mes_class):
        ...
        for directive in directives:
            ...
            docString = None
            docString = (
                Registry.instance.class_list[mes_class]["class"]
                ._enabled_directives[directive]
                .__doc__
            )
            if (
                directive
    ...
    def get_mes_component_properties(self, mes_class):
        mes_component_type = None
        property_list = []
        properties = Registry.instance.class_list[mes_class][
            "class"
        ]._mtree_properties.keys()
        for property_name in properties:
            property_list.append(
                (
                    property_name,
                    Registry.instance.class_list[mes_class]["class"]._mtree_properties[
                        property_name
                    ],
                )
            )
